...
import { IconEye, IconPlus, IconSearch, IconTrash, IconX } from "@tabler/icons-react";
...
export function Promotions() {
    ...
    const { isLoadingPromotions, isRefetchingPromotions, promotions } = usePromotion(false);
    ...
    return isLoadingPromotions ? (
        <PageLoading />
    ) : (
        <section className="flex flex-col gap-y-3">
            <h1 className="font-bold text-2xl">Promotions Management</h1>
            <div className="flex items-center justify-between gap-5">
                ...
                <div className="flex items-center justify-end gap-4 w-fit">
                    <Button
                        type="button"
                        onClick={() => router.push("/admin/promotions/trash")}
                        className="bg-yellow text-red font-semibold"
                    >
                        <IconTrash stroke={2} size={20} /> <span>Trash</span>
                    </Button>
                    <Button
                            ...
                            >
                                {hover === p.code && (
                                    <div className="absolute top-0 left-0 right-0 w-full h-full bg-black/20 backdrop-blur-sm flex items-center justify-center">
                                        ...
                                        <Button
                                            type="button"
                                            className="w-fit scale-75 bg-red text-gray-50"
        ...
        <div className="fixed top-0 left-0 right-0 w-full h-screen z-30 flex items-center justify-center bg-black/20 backdrop-blur-sm">
            <Card className="flex flex-col gap-y-5 w-4/12">
                <p className="text-center text-sm text-gray-500">
                    Are you sure you want to deleted this promotion {code}?
                </p>
